...
const ResultRow: React.FC<ResultRowProps> = ({ match }) => {
    ...
    return (
        <Link href={`/matches/${match.id}`} className="block crystal-glass rounded-2xl overflow-hidden crystal-hover mb-4 relative">
            {/* Status Badge */}
            ...
            <div className="p-6 relative">
                <div className="flex items-center justify-between gap-6">
                    {/* Home Team */}
                    <div className={`flex-1 flex items-center gap-4 transition-opacity ${!homeWon && !draw ? 'opacity-50' : ''}`}>
                        <div className={`w-12 h-12 relative bg-white/10 rounded-xl p-2 flex-shrink-0 ${homeWon ? 'ring-2 ring-green-400 ring-offset-2 ring-offset-transparent' : ''}`}>
                            {homeTeamData?.logo ? (
                                <Image
                        ...
                        <div className="flex-1">
                            <div className="flex items-center gap-2">
                                ...
                                {homeWon && <span className="text-green-400 font-bold">✓</span>}
                            </div>
                    ...
                    <div className="flex flex-col items-center gap-2 min-w-[120px]">
                        <div className="gradient-mwiri rounded-xl px-6 py-3 shadow-lg">
                            <p className="text-white font-black text-2xl text-center flex items-center gap-3">
                                <span className={homeWon ? 'text-mwiri-gold' : ''}>{match.homeScore ?? 0}</span>
                                <span className="text-white/50 text-base">-</span>
                                <span className={awayWon ? 'text-mwiri-gold' : ''}>{match.awayScore ?? 0}</span>
                            </p>
                        </div>
                    ...
                    {/* Away Team */}
                    <div className={`flex-1 flex items-center gap-4 flex-row-reverse transition-opacity ${!awayWon && !draw ? 'opacity-50' : ''}`}>
                        <div className={`w-12 h-12 relative bg-white/10 rounded-xl p-2 flex-shrink-0 ${awayWon ? 'ring-2 ring-green-400 ring-offset-2 ring-offset-transparent' : ''}`}>
                            {awayTeamData?.logo ? (
                                <Image
                        ...
                        <div className="flex-1 text-right">
                            <div className="flex items-center justify-end gap-2">
                                {awayWon && <span className="text-green-400 font-bold">✓</span>}
